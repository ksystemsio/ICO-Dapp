...
const LogiqTokenCrowdsale = artifacts.require('LogiqTokenCrowdsale')
const LogiqToken = artifacts.require('LogiqToken')

contract('LogiqTokenCrowdsale', function ([owner, wallet, investor]) {
  ...
  beforeEach(async function () {
    this.crowdsale = await LogiqTokenCrowdsale.new(
  ...
  it('should allow admin to change token owner', async function () {
    ...
    const ownerOld = await LogiqToken.at(tokenAddr).owner()
    await this.crowdsale.transferTokenOwnership(investor).should.be.fulfilled;
    const ownerNew = await LogiqToken.at(tokenAddr).owner()
  ...
  it('should accept payments during the sale for whitelisted address', async function () {
    ...
    const balance = await LogiqToken.at(token).balanceOf(investor)
    ...
    const balance2 = await LogiqToken.at(token).balanceOf(investor)
    ...
    await this.crowdsale.buyTokens(investor, { value: ether(0.05), from: investor }).should.be.fulfilled
    const balance3 = await LogiqToken.at(token).balanceOf(investor)
    ...
    await this.crowdsale.buyTokens(investor, { value: ether(0.05), from: investor }).should.be.fulfilled
    const balance4 = await LogiqToken.at(token).balanceOf(investor)
    ...
    await this.crowdsale.buyTokens(investor, { value: ether(0.05), from: investor }).should.be.fulfilled
    const balance5 = await LogiqToken.at(token).balanceOf(investor)
    ...
    const balance6 = await LogiqToken.at(token).balanceOf(investor)
